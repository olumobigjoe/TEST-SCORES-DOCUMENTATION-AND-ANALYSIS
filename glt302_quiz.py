...
.qcard { background:#f8f9fa; border:1px solid #dee2e6;
         border-radius:10px; padding:18px 20px; margin-bottom:14px; font-size:1.05rem; }
...
.fail-box { background:#f8d7da; border:2px solid #842029; color:#842029;
            padding:26px; border-radius:14px; text-align:center; }
.big-score { font-size:3.2rem; font-weight:700; line-height:1.2; }
...
if st.session_state.page == "login":
    ...
    name_input = st.text_input("Enter your Full Name:", placeholder="e.g. Adaeze Okonkwo")
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    answered_n = len(st.session_state.answers)
    ...
    st.markdown(f"**Question {q_idx + 1} of {NUM_QUESTIONS}** &nbsp;|&nbsp; "
                f"✅ Answered: {answered_n} / {NUM_QUESTIONS}")
    st.progress(q_idx / NUM_QUESTIONS)
    st.write("")
    ...
    if q_idx < NUM_QUESTIONS:
        ...
        if not confirmed:
            ...
            for opt in q_data["shuffled_options"]:
                ...
                if st.button(f"{prefix}  {opt}", key=f"opt__{q_idx}__{opt}"):
                    st.session_state.selected_opt = opt
                    st.rerun()

            st.write("")
            if st.session_state.selected_opt is not None:
                st.info(f"**Selected:** {st.session_state.selected_opt}")
                if st.button("✔  Confirm Answer", type="primary"):
                    chosen     = st.session_state.selected_opt
                    is_correct = (chosen == q_data["answer"])
                    st.session_state.answers[q_idx]  = chosen
                    st.session_state.feedback[q_idx] = is_correct
                    st.session_state.confirmed       = True
                    st.rerun()
            else:
                st.caption("👆 Select one option above, then click Confirm Answer.")

        else:
            # Show result of this question
            chosen     = st.session_state.answers[q_idx]
            is_correct = st.session_state.feedback[q_idx]

            for opt in q_data["shuffled_options"]:
                if opt == q_data["answer"] and opt == chosen:
                    st.markdown(f"✅ **{opt}** ← your answer (correct)")
                elif opt == q_data["answer"]:
            ...
            st.write("")
            if is_correct:
                st.markdown('<div class="fb-correct">✔️ Correct! Well done.</div>',
                            unsafe_allow_html=True)
            else:
                st.markdown(
                    f'<div class="fb-wrong">❌ Wrong. '
                    f'Correct answer: <strong>{q_data["answer"]}</strong></div>',
                    unsafe_allow_html=True)
    ...
    # ── Early submit ──────────────────────────────────────────────────────────
    if answered_n > 0 and q_idx < NUM_QUESTIONS - 1:
        st.divider()
        with st.expander("⚠️  End test early?"):
            st.warning(f"You have answered {answered_n} of {NUM_QUESTIONS} questions. "
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if passed:
        ...
        <div class="pass-box">
            ...
            <span style="font-size:.95rem;">
                ...
                <strong>{NUM_QUESTIONS}</strong> correctly.
            </span>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
    ...
    for i, q_data in enumerate(questions):
        ...
        wrong_note = "" if is_correct else f"<br><strong>✔ Correct:</strong> {q_data['answer']}"
        st.markdown(f"""
        <div style="background:{bg}; border-radius:8px; padding:12px 16px;
                    margin-bottom:8px; font-size:.93rem;">
            {icon} <strong>Q{i+1}. {q_data['q']}</strong><br>
            <em>Your answer: {chosen}</em>{wrong_note}
        </div>""", unsafe_allow_html=True)
